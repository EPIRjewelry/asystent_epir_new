...
8. Integracja LLM (`worker/src/ai-client.ts`)
   - Werdykt: ZGODNE. Cały worker korzysta wyłącznie z modelu `gpt-oss-120b` przez Groq API. Nie ma możliwości użycia innych modeli.
...
Priorytet 2 — Utrzymanie wyłączności modelu:
- Cały kod korzysta wyłącznie z modelu `gpt-oss-120b` przez Groq API.
